...
//grind lines start here
/*
GridGeometry = new Array();
		  
		for (var x = 0; x < width; x++) {
			GridGeometry[x] = new Array();
			for (var y = 0; y < height; y++) {
				GridGeometry[x][y] = new THREE.Geometry();
			    GridGeometry[x][y].vertices.push(new THREE.Vertex(new THREE.Vector3((x*widthOfTile) +xoffset,2, (y*heightOfTile)+yoffset)));
			    GridGeometry[x][y].vertices.push(new THREE.Vertex(new THREE.Vector3((x*widthOfTile)+widthOfTile+xoffset,2, (y*heightOfTile)+yoffset)));
			    GridGeometry[x][y].vertices.push(new THREE.Vertex(new THREE.Vector3((x*widthOfTile)+widthOfTile+xoffset,2,(y*widthOfTile)+heightOfTile+yoffset)));
			     GridGeometry[x][y].vertices.push(new THREE.Vertex(new THREE.Vector3((x*widthOfTile)+xoffset,2, (y*heightOfTile)+heightOfTile+yoffset)));
				  if( this.grid[x][y] === 1)
				  {
				  	GridGeometry[x][y].vertices.push(new THREE.Vertex(new THREE.Vector3((x*widthOfTile) +xoffset,2, (y*heightOfTile)+yoffset)));
				    GridGeometry[x][y].vertices.push(new THREE.Vertex(new THREE.Vector3((x*widthOfTile)+widthOfTile+xoffset,2,(y*widthOfTile)+heightOfTile+yoffset)));
				    scene.add(new THREE.Line(GridGeometry[x][y],lineRedmaterial)); 
				  }
				  else
				  {
				    scene.add(new THREE.Line(GridGeometry[x][y],linematerial)); 
				  }
			}
		}
	...
	this.findPath = function(startXLocation,startYLocation,endXLocation,endYLocation)
	{
		...
		while(true)
		{
			Points.push(new THREE.Vector3((searchNode.nodex * widthOfTile) +xoffset + (widthOfTile/2) ,0,(searchNode.nodey*heightOfTile)+ yoffset+(heightOfTile/2)));
			    ptdebug.vertices.push(new THREE.Vertex(new THREE.Vector3((searchNode.nodex * widthOfTile) +xoffset + (widthOfTile/2) ,5,(searchNode.nodey*heightOfTile)+ yoffset+(heightOfTile/2))));
