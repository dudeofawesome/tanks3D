...
function AiTanks(){
	...
	this.pathing = new PathFinding(527,527,-2734,-2734,10,10);
	...
	this.AiTankSetup = function(){
        ...
        model.load("../models/tankBody.json",function(bodyGeo){
            ...
            for(var counter=0;counter<(numberOfEnemies*1);counter++){
                ...
                enemytanks.GetTankArray()[counter].tankBody=new THREE.SkinnedMesh(bodyGeo,material);
                //this.tanks[counter].tankBody.doubleSided=true;
                enemytanks.GetTankArray()[counter].tankBody.scale.set(5,5,5);
                enemytanks.GetTankArray()[counter].tankBody.castShadow = true;
                enemytanks.GetTankArray()[counter].tankBody.receiveShadow = true;
                enemytanks.GetTankArray()[counter].tankBody.position.z=-500;
                if(counter==0){
                    enemytanks.GetTankArray()[counter].tankBody.position.x=-20*(numberOfEnemies-1);
                }
                else{
                    enemytanks.GetTankArray()[counter].tankBody.position.x=enemytanks.GetTankArray()[counter-1].tankBody.position.x+20;
                }
            }
        });
        model.load("../models/tankTurret.json",function(turretGeo){
            for(var counter=0;counter<(numberOfEnemies*1);counter++){
                ...
                var material=new THREE.MeshLambertMaterial({color:0xffffff});
                enemytanks.GetTankArray()[counter].tankTurret=new THREE.SkinnedMesh(turretGeo,material);
                enemytanks.GetTankArray()[counter].tankTurret.scale.set(5,5,5);
                enemytanks.GetTankArray()[counter].tankTurret.castShadow = true;
                enemytanks.GetTankArray()[counter].tankTurret.receiveShadow = true;
                enemytanks.GetTankArray()[counter].tankTurret.position=enemytanks.GetTankArray()[counter].tankBody.position;

                scene.add(enemytanks.GetTankArray()[counter].tankBody);
                scene.add(enemytanks.GetTankArray()[counter].tankTurret);
           }
        });
      ...
      this.pathing.setGroupOfPointsAsOccupied(_playerDeadZone);
        ...
        for (var i = this.tanks.length - 1; i >= 0; i--) {
        					...
        					(this.tanks[i].tankBody.position.z-this.tanks[i].getPointToGoTo().z))) < 15)
            {
                ...
                if(this.tanks[i].index > this.tanks[i].point.length-1)
                {
                	var rotation = Math.random() * 3.14;
                	 this.tanks[i].point = this.pathing.findPath(this.pathing.getXLocation(this.tanks[i].tankBody.position.x),
                	 this.pathing.getYLocation(this.tanks[i].tankBody.position.z),
                	 this.pathing.getXLocation(tankYou.model.position.x  - (Math.cos(rotation)*100)),
                	 this.pathing.getYLocation(tankYou.model.position.z- (Math.sin(rotation)*100)));
        	...
        	if(distance<=shootingDistance){
                ...
                shells[shells.length-1].vX=Math.sin(this.tanks[i].tankTurret.rotation.y-Math.PI/2)*25+Math.random() * (shootingDistance/accuracy-shootingDistance/-accuracy)+shootingDistance/-accuracy;
                shells[shells.length-1].vY=0;
                shells[shells.length-1].vZ=Math.cos(this.tanks[i].tankTurret.rotation.y-Math.PI/2)*25+Math.random() * (shootingDistance/accuracy-shootingDistance/-accuracy)+shootingDistance/-accuracy;
                shells[shells.length-1].model=new THREE.Mesh(new THREE.SphereGeometry(1,3,3), new THREE.MeshLambertMaterial({color: 0xE3B707}));
                shells[shells.length-1].model.scale.set(.2,.2,.2);
                shells[shells.length-1].model.position.x=this.tanks[i].tankTurret.position.x;
                shells[shells.length-1].model.position.y=4;
                shells[shells.length-1].model.position.z=this.tanks[i].tankTurret.position.z;
